...
/** Processes streamed JSON chunks and emits complete {@link GeminiResponse} objects. */
@Slf4j
public class ResponseStreamProcessor {
  ...
  @Getter private final Queue<GeminiResponse> responseQueue = new ConcurrentLinkedQueue<>();
            ...
            .forEach(
                candidate ->
                    candidate
                        .content()
                        .parts()
                        .forEach(part -> log.info("Text part: {}", part.text())));
