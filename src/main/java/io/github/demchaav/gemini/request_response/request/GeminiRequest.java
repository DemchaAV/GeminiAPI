...
public record GeminiRequest(
    /**
     * Optional: string The name of the cached content used as context to serve the prediction.
     * Format: projects/{project}/locations/{location}/cachedContents/{cachedContent}
     */
    String cachedContent,
    /**
     * Required: Content The content of the current conversation with the model. For single-turn
     * queries, this is a single instance. For multi-turn queries, this is a repeated field that
     * contains conversation history and the latest request.
     */
    List<Content> contents,
    /**
     * Optional: Content Available for gemini-1.5-flash, gemini-1.5-pro, and gemini-1.0-pro-002.
     * Instructions for the model to steer it toward better performance. For example, "Answer as
     * concisely as possible" or "Don't use technical terms in your response". The text strings
     * count toward the token limit. The role field of systemInstruction is ignored and doesn't
     * affect the performance of the model.
     */
    SystemInstruction systemInstruction,
    /**
     * Optional. A piece of code that enables the system to interact with external systems to
     * perform an action, or set of actions, outside of knowledge and scope of the model. See
     * Function calling.
     */
    List<Tool> tools,
  ...
  public static GeminiRequest requestMessage(@NonNull Message message) {
    ...
    return GeminiRequest.builder().addContent(new Content("user", message.text())).build();
  }
