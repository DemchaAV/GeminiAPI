...
@Slf4j
/**
 * Utility for querying and logging the list of models visible to the configured API key.
 *
 * @since 1.0.4
 */
public class GeminiModelLister {
  ...
  /**
   * Creates a model lister for the supplied API key.
   *
  ...
  /**
   * Fetches the raw JSON response from the Gemini models endpoint.
   *
   * @return the raw JSON payload, or {@code null} when the request fails
   * @since 1.0.4
   */
  public String listModelsJson() {
    ...
    HttpRequest request =
        HttpRequest.newBuilder().uri(requestUri).timeout(Duration.ofSeconds(10)).GET().build();
  ...
  /**
   * Parses a model-list payload and logs the discovered models.
   *
   * @param jsonResponse raw JSON payload returned by {@link #listModelsJson()}
   * @since 1.0.4
   */
