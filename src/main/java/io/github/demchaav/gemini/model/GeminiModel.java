 ...
 * "gemini-2.0-flash"
 *
 * <p><strong>Inputs:</strong> Audio, images, videos, and text
 *
 * <p><strong>Outputs:</strong> Text, images (coming soon), and audio (coming soon)
 *
 * <p><strong>Features:</strong> Next-generation capabilities, high speed, and multimodal generation
 * for a wide range of tasks. "gemini-2.0-flash-lite"
 *
 * <p><strong>Inputs:</strong> Audio, images, videos, and text
 *
 ...
 * fewer computational resources. "gemini-1.5-pro"
 *
 * <p><strong>Inputs:</strong> Audio, images, videos, and text
 *
 * <p><strong>Outputs:</strong> Text
...
public class GeminiModel
    implements ModelType<GeminiVariation, GeminiVersion, GeminiGenerateMethod> {
  ...
  @Override
  public GeminiVariation getVariation() {
    return variation;
  }

  @Override
  public GeminiVersion getVersion() {
    return version;
  }

  @Override
  public GeminiGenerateMethod getGenerateMethod() {
  ...
  public String getUrl() {
    ...
    if (generateMethod == GeminiGenerateMethod.STREAM_GENERATE_CONTENT) {
      return "%s/%s/models/%s-%s-%s:%s?alt=sse&key="
          .formatted(BASE_URL, verAPI, getModelName(), variation, version, generateMethod);
    }
    return "%s/%s/models/%s-%s-%s:%s?key="
        .formatted(BASE_URL, verAPI, getModelName(), variation, version, generateMethod);
  }
  ...
  public static class GeminiModelBuilder {
    ...
    public GeminiModelBuilder copyModelAndSetGenerateMethod(
        GeminiModel model, GeminiGenerateMethod generateMethod) {
      this.verAPI = model.getVerAPI();
      this.variation = model.getVariation();
      this.version = model.getVersion();
      this.generateMethod = generateMethod;
