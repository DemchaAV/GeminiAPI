...
/**
 * Class for handling API errors with appropriate error codes and messages. Contains error codes
 * from HTTP status codes and canonical error codes.
 */
public class ApiErrorHandler {
  ...
  private static void initErrorMappings() {
    ...
    ERROR_CAUSES.put(
        400,
        "Request fails API validation, or you tried to access a model that requires"
            + " allowlisting or is disallowed by the organization's policy.");
    ...
    ERROR_CAUSES.put(
        429,
        "API quota over the limit, server overload due to shared server capacity, or daily limit for requests using logprobs reached.");
    ...
    ERROR_CAUSES.put(
        504,
        "The client sets a deadline shorter than the server's default deadline"
            + " (10 minutes), and the request didn't finish within the"
            + " client-provided deadline.");

    // Error solutions
    ERROR_SOLUTIONS.put(
        400,
        "Refer to the GeminiModel API reference for Generative AI for request parameters, token count, and other parameters.");
    ERROR_SOLUTIONS.put(
        403,
        "Verify that all necessary APIs are enabled, and the service account has"
            + " the right permission to access the selected service. Ensure"
            + " P4SA is granted necessary permission to access referenced resources.");
    ERROR_SOLUTIONS.put(404, "Check and fix the file location.");
    ERROR_SOLUTIONS.put(
        429,
        "Check quota limits. If needed, apply for a higher quota. Retry after a few seconds. If the error persists, contact support.");
    ERROR_SOLUTIONS.put(499, "Review client-side request handling.");
    ERROR_SOLUTIONS.put(
        500,
        "Retry after a few seconds. If the error persists after a prolonged period of time (hours), contact support.");
    ERROR_SOLUTIONS.put(
        503, "The unavailable status might be temporary. If the error persists, contact support.");
    ERROR_SOLUTIONS.put(
        504,
        "Increase client timeout or optimize the request to complete within the default deadline.");
  }
    ...
    public ApiError(int httpErrorCode) {
      ...
      super(
          "API Error: "
              + httpErrorCode
              + " - "
              + ApiErrorHandler.getCanonicalErrorCode(httpErrorCode));
